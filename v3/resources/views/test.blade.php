...
@section('content')
    ...
    @if (Session::has('message'))
        <div class=" border-green-600 rounded-xl translation hover:bg-green-500 duration-300 m-8 p-3 text-green-600">
    ...
    <form name="form" method='POST' action='{{ route('test.store') }}' id="form">
        ...
        <dir>
            ...
            <input name="full_name" id="full_name" type=text maxlength=30
                class="colortext_test rounded-md border-rose-400 block" value="{{ old('full_name') }}">
            @error('full_name')
        ...
        <dir>
            ...
            <input name="email" id="email" type='text' maxlength=30
                class="colortext_test  rounded-md border-rose-400 block" value="{{ old('email') }}">
            @error('email')
                <span class="text-red-500 text-sm block font-bold">{{ $message }}</span>
            @enderror
        </dir>
        <dir>
            ...
            <textarea name="question_1" id="question_1" class="colortext_test  rounded-md border-rose-400 block"
                cols="90">
@if (old('question_1'))
{{ old('question_1') }}
@endif
